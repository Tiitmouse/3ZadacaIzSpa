...
void draw_map(const unsigned short i_x, const unsigned short i_y, const gbl::Position<>& i_finish_position,
              const gbl::Position<>& i_start_position, sf::RenderWindow& i_window, sf::Sprite& i_map_sprite,
              const gbl::Map<>& i_map, const bool i_change_colors, const gbl::Map<float>& i_distances)
{
    ...
    for (unsigned short a = 0; a < gbl::MAP::COLUMNS; a++)
    {
        ...
        {
            i_map_sprite.setPosition(gbl::MAP::ORIGIN_X + i_x + a * gbl::MAP::CELL_SIZE,
                                     gbl::MAP::ORIGIN_Y + i_y + b * gbl::MAP::CELL_SIZE);
            ...
            else
            {
                ...
                switch (cell_type)
                {
                    ...
                    {
                        i_map_sprite.setColor(sf::Color(200, 200, 200));

                        break;
                    }
                    case gbl::MAP::Cell::Path:
                    {
                        i_map_sprite.setColor(gbl::COLOR::PATHCOLOR);
                    ...
                    case gbl::MAP::Cell::Visited:
                    {
                        if (0 != i_change_colors)
                        {
                            ...
                            i_map_sprite.setColor(sf::Color(0, std::max(0, color_value - 170),
                                                            std::min(255, 85 + color_value)));
                        }
                        else
                        {
                    ...
                    {
                        i_map_sprite.setColor(gbl::COLOR::WALLCOLOR);
                    }
                    case gbl::MAP::Invalid:
                        break;
                }
            }
